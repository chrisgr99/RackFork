...
/** Per-panel dimming strength for one module type — see design/panel-dimming.md.

Reads as "auto" until the user moves it, at which point an override is stored against
this plugin and module slug in settings.json. Storing only what the user actually
disagreed with is what keeps this workable across hundreds of modules.
*/
struct PanelDimPanelQuantity : Quantity {
	std::string pluginSlug;
	std::string moduleSlug;

	PanelDimPanelQuantity(const std::string& pluginSlug, const std::string& moduleSlug)
		: pluginSlug(pluginSlug), moduleSlug(moduleSlug) {}

	void setValue(float value) override {
		settings::setPanelDimStrength(pluginSlug, moduleSlug, math::clamp(value, 0.f, 1.f));
		window::panelDimRefresh();
	}
	float getValue() override {
		float stored = settings::getPanelDimStrength(pluginSlug, moduleSlug);
		// With no override, show the middle of the range rather than zero: zero would
		// read as "this panel is not being dimmed", which is not what auto means.
		return stored >= 0.f ? stored : 0.5f;
	}
	...
	std::string getDisplayValueString() override {
		if (settings::getPanelDimStrength(pluginSlug, moduleSlug) < 0.f)
			return "auto";
		return Quantity::getDisplayValueString();
	}
};
struct PanelDimPanelSlider : ui::Slider {
	PanelDimPanelSlider(const std::string& pluginSlug, const std::string& moduleSlug) {
		quantity = new PanelDimPanelQuantity(pluginSlug, moduleSlug);
	}
	~PanelDimPanelSlider() {
		delete quantity;
	}
...
void ModuleWidget::draw(const DrawArgs& args) {
	...
	// Drawn knobs and jacks, on top of whatever artwork the plugin shipped.
	// This must live here rather than in an override on Knob: a plugin subclass's vtable
	// was generated by its own compiler, so an override we add now would never run for
	// third-party controls. See design/control-appearance.md.
	appearance::drawControls(this, args);
...
void ModuleWidget::createContextMenu() {
	...
	// Per-panel brightness reduction — see design/panel-dimming.md.
	// Only offered when the feature is on, so it stays out of the way otherwise.
	if (settings::panelDimEnabled && model && model->plugin) {
		const std::string pluginSlug = model->plugin->slug;
		const std::string moduleSlug = model->slug;
		const float stored = settings::getPanelDimStrength(pluginSlug, moduleSlug);

		menu->addChild(new ui::MenuSeparator);
		menu->addChild(createSubmenuItem("Panel brightness", stored >= 0.f ? "custom" : "auto",
			[=](ui::Menu* menu) {
				menu->addChild(createMenuLabel("How hard to dim this panel specifically."));
				menu->addChild(new ui::MenuSeparator);

				PanelDimPanelSlider* slider = new PanelDimPanelSlider(pluginSlug, moduleSlug);
				slider->box.size.x = 220.0;
				menu->addChild(slider);

				menu->addChild(createMenuItem("Back to automatic", "", [=]() {
					settings::clearPanelDimStrength(pluginSlug, moduleSlug);
					window::panelDimRefresh();
				}, stored < 0.f));
			}
		));
